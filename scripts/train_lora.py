...
from s1proto.template import DECIDE, OPT_END, render
...
def build_batch(tok, items, rng, device, max_length: int, train: bool, layout: str = "letters", proc=None, image_root=None):
    ...
    texts, refs, nopts, images = [], [], [], []
    for it in items:
        q, ref = shuffled(it, rng) if train else (it["question"], [it["ref"][k] for k in option_keys(it["question"])])
        if proc is not None:  # vision: the image is the state; any text rides along as a caption
            ...
            images.append(Image.open(Path(image_root) / it["state"]["image"]).convert("RGB"))
        else:
    ...
    if layout == "pointer":
        end_id, dec_id = tok.convert_tokens_to_ids(OPT_END), tok.convert_tokens_to_ids(DECIDE)
...
def hidden_states(model, enc, modality="text"):
    ...
    if modality == "vision":
        return body(**{k: v for k, v in enc.items() if k in ("input_ids", "attention_mask", "pixel_values", "image_grid_thw", "mm_token_type_ids")}, use_cache=False).last_hidden_state
...
def evaluate(model, head, tok, items, device, batch, max_length, layout="letters", proc=None, image_root=None, modality="text"):
    ...
    for s in range(0, len(items), batch):
        ...
        enc, ref, nopts, opt_pos, dec_pos = build_batch(tok, chunk, rng, device, max_length, train=False, layout=layout, proc=proc, image_root=image_root)
...
def main() -> None:
    ...
    ap.add_argument("--head", choices=["pointer", "slot"], default="pointer", help="pointer: query/key readout over option delimiters (no option cap); slot: fixed 256-way linear head")
    ...
    ap.add_argument("--modality", choices=["text", "vision"], default="text", help="vision: a vision-language base (Qwen3-VL); items carry state.image relative to the data file's directory")
    ...
    targets = r".*language_model.*\.(q_proj|k_proj|v_proj|o_proj|gate_proj|up_proj|down_proj)" if args.modality == "vision" else ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"]
    ...
    def length_of(it: dict) -> int:
        state = (it["state"].get("text") or "See the image.") if args.modality == "vision" else it["state"]
        return len(tok(render(state, parse_question(it["question"]), layout=layout).text)["input_ids"]) + (300 if args.modality == "vision" else 0)
    ...
    for epoch in range(args.epochs):
        for chunk in bucketed_batches(train):
            if skip > 0:
                ...
                step += 1
                continue
            enc, ref, nopts, opt_pos, dec_pos = build_batch(tok, chunk, rng, device, args.max_length, train=True, layout=layout, proc=proc, image_root=image_root)
            ...
            if step % args.eval_every == 0 or step == total_steps:
                ...
                if m["ece"] < best["ece"]:
                    ...
                    json.dump(
                        {"base": args.model, "hidden": hidden, "head": args.head, "head_size": HEAD_SIZE, "head_dim": HEAD_DIM, "layout": layout, "load_4bit": args.load_4bit, "modality": args.modality, "best": best, "args": vars(args)},
