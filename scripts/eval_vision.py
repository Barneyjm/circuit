"""Score a vision-grid JSONL with a raw vision-language model by letter
logits (Phase 1), reporting the same metrics as every other row.

    uv run python scripts/eval_vision.py data/vision/grid/eval.jsonl --out results/vgrid_qwen3vl4b_raw.json
    uv run python scripts/eval_vision.py data/vision/grid/eval.jsonl --model Qwen/Qwen3-VL-8B-Instruct --out ...

State is {"image": <path relative to the JSONL's directory>, "text": <optional>}.
Options are lettered A.. (26 max); the next-token distribution over the
letters is the prediction.
"""
...
from vision_probe import LETTERS, letter_ids, load
...
def option_texts(q: dict) -> list[str]:
    if q["type"] == "noul":
        c = q.get("criteria") or {}
        return [f"Yes{': ' + c['true'] if c.get('true') else ''}", f"No{': ' + c['false'] if c.get('false') else ''}"]
    if q["type"] == "choice":
        return [f"{k}{': ' + str(v) if v else ''}" for k, v in q["criteria"].items()]
    return [str(level) for level in q["criteria"]]
...
def score_lora(run_dir: str, items: list, base_dir: Path, device: str):
    """Score with a trained vision run: the trainer's own batching and head, one item at a time."""
    import random
    ...
    base = AutoModelForImageTextToText.from_pretrained(cfg["base"], dtype=torch.bfloat16)
