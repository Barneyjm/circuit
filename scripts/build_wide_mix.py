...
TASKS: list[Task] = [
    ...
    Task(
        ...
        "nyu-mll/glue",
        "noul",
        "Is the sentiment of this movie-review sentence positive?",
        ["yes", "no"],
        "label",
        config="sst2",
        eval_split="validation",
        criteria={"true": "Positive", "false": "Negative"},
        state=lambda r: r["sentence"],
        label=lambda r: [1, 0][r["label"]] if r["label"] >= 0 else None,
        license="unclear",
    ),
    Task(
        "imdb",
        "stanfordnlp/imdb",
        "noul",
        "Is this movie review positive overall?",
        ["yes", "no"],
        "label",
        criteria={"true": "Positive", "false": "Negative"},
        state=lambda r: r["text"][:1500],
        label=lambda r: [1, 0][r["label"]],
        license="unclear",
    ),
    Task(
        "rotten_tomatoes",
        ...
        "noul",
        "Is this review snippet positive?",
        ["yes", "no"],
        "label",
        criteria={"true": "Positive", "false": "Negative"},
        label=lambda r: [1, 0][r["label"]],
    ...
    Task(
        ...
        "label",
        criteria={"true": "Positive", "false": "Negative"},
        state=lambda r: {"title": r["title"], "review": r["content"][:1500]},
        label=lambda r: [1, 0][r["label"]],
        license="unclear",
    ),
    Task(
        ...
        ["0", "1", "2", "3", "4"],
        "label",
        criteria=["1 star: terrible", "2 stars: poor", "3 stars: okay", "4 stars: good", "5 stars: excellent"],
        state=lambda r: r["text"][:1500],
        label=idx("label"),
        license="nc",
    ),
    ...
    Task(
        ...
        "choice",
        "What is the sentiment of this financial news sentence from an investor's point of view?",
        [],
        "label",
        eval_split="train",
        label=idx("label"),
        label_names_field="label_text",
        license="nc",
    ),
    Task(
        "tweet_sentiment",
        ...
        "label",
        config="sentiment",
        criteria=choice(["negative", "neutral", "positive"]),
        label=idx("label"),
        license="nc",
    ),
    # --- emotion
    Task(
        ...
        "label",
        config="split",
        criteria=choice(["sadness", "joy", "love", "anger", "fear", "surprise"]),
        label=idx("label"),
        license="nc",
    ),
    Task(
        ...
        "label",
        config="emotion",
        criteria=choice(["anger", "joy", "optimism", "sadness"]),
        label=idx("label"),
        license="nc",
    ),
    ...
    Task(
        ...
        "cardiffnlp/tweet_eval",
        "noul",
        "Is this tweet hateful (attacks a group on identity)?",
        ["yes", "no"],
        "label",
        config="hate",
        criteria={"true": "Hateful", "false": "Not hateful"},
        label=lambda r: [1, 0][r["label"]],
        license="nc",
    ),
    Task(
    ...
    Task(
        ...
        "label",
        config="irony",
        criteria={"true": "Ironic", "false": "Literal"},
        label=lambda r: [1, 0][r["label"]],
        license="nc",
    ),
    Task(
        ...
        "cardiffnlp/tweet_eval",
        "choice",
        "What is this tweet's stance toward legalized abortion?",
        ["none", "against", "favor"],
        "label",
        config="stance_abortion",
        criteria=choice(["none", "against", "favor"], {"none": "No stance expressed"}),
        label=idx("label"),
        license="nc",
    ),
    Task(
        ...
        "cardiffnlp/tweet_eval",
        "choice",
        "What is this tweet's stance toward the claim that climate change is a real concern?",
        ["none", "against", "favor"],
        "label",
        config="stance_climate",
        criteria=choice(["none", "against", "favor"], {"none": "No stance expressed"}),
        label=idx("label"),
        license="nc",
    ),
    ...
    Task(
        ...
        config="rte",
        eval_split="validation",
        criteria={"true": "Entailed", "false": "Not entailed"},
        state=lambda r: {"premise": r["sentence1"], "hypothesis": r["sentence2"]},
        label=lambda r: [1, 0][r["label"]] if r["label"] >= 0 else None,
        license="unclear",
    ),
    Task(
        ...
        "stanfordnlp/snli",
        "choice",
        "Given `premise`, what is the status of `hypothesis`?",
        ...
        state=lambda r: {"premise": r["premise"], "hypothesis": r["hypothesis"]},
        label=idx("label"),
        license="ok",
    ),
    Task(
        ...
        config="qqp",
        eval_split="validation",
        criteria={"true": "Duplicates", "false": "Different questions"},
        state=lambda r: {"question1": r["question1"], "question2": r["question2"]},
        label=lambda r: [1, 0][r["label"]] if r["label"] >= 0 else None,
        license="nc",
    ),
    Task(
        ...
        config="mrpc",
        eval_split="validation",
        criteria={"true": "Paraphrases", "false": "Not paraphrases"},
        state=lambda r: {"sentence1": r["sentence1"], "sentence2": r["sentence2"]},
        label=lambda r: [1, 0][r["label"]],
        license="nc",
    ),
    Task(
        ...
        criteria={"true": "Same meaning", "false": "Different meaning (word overlap is not enough)"},
        state=lambda r: {"sentence1": r["sentence1"], "sentence2": r["sentence2"]},
        label=lambda r: [1, 0][r["label"]],
        license="ok",
    ),
    Task(
        ...
        "nyu-mll/glue",
        "noul",
        "Does `sentence` contain the answer to `question`?",
        ["yes", "no"],
        "label",
        config="qnli",
        eval_split="validation",
        criteria={"true": "Contains the answer", "false": "Does not"},
    ...
    Task(
        ...
        state=lambda r: {"question": r["question"], "passage": r["passage"][:1500]},
        label=lambda r: 0 if r["answer"] else 1,
        license="ok",
    ),
    Task(
        "wiki_qa",
        "microsoft/wiki_qa",
        "noul",
        "Does `sentence` answer `question`?",
        ["yes", "no"],
        "label",
        criteria={"true": "Answers it", "false": "Does not answer it"},
        state=lambda r: {"question": r["question"], "sentence": r["answer"]},
        label=lambda r: [1, 0][r["label"]],
        license="unclear",
    ),
    ...
    Task(
        ...
        "SetFit/subj",
        "noul",
        "Is this sentence subjective (an opinion) rather than objective (a fact)?",
        ["yes", "no"],
        "label",
        criteria={"true": "Subjective", "false": "Objective"},
        label=lambda r: [1, 0][r["label"]],
        license="unclear",
    ),
    Task(
        ...
        "coarse_label",
        criteria={
            "abbreviation": "An abbreviation or its expansion",
            "entity": "A thing (animal, color, product, ...)",
            "description": "A definition, reason, or explanation",
            "human": "A person or group",
            "location": "A place",
            "numeric": "A number, date, or amount",
        },
        label=idx("coarse_label"),
    ...
    Task(
        ...
        "fancyzhx/ag_news",
        "choice",
        "Which section does this news article belong in?",
        ["world", "sports", "business", "sci_tech"],
        "label",
        criteria=choice(["world", "sports", "business", "sci_tech"]),
        label=idx("label"),
        license="nc",
    ),
    Task(
        ...
        "fancyzhx/dbpedia_14",
        "choice",
        "What kind of entity is this encyclopedia article about?",
        [
            "company",
        ...
        state=lambda r: {"title": r["title"], "content": r["content"][:1200]},
        label=idx("label"),
        license="ok",
    ),
    Task(
        ...
        state=lambda r: {"title": r["question_title"], "body": r["question_content"][:800]},
        label=idx("topic"),
        license="nc",
    ),
    Task(
        ...
        "choice",
        "Which newsgroup was this post made to?",
        [],
        "label",
        state=lambda r: r["text"][:1500],
        label=idx("label"),
    ...
    Task(
        ...
        "choice",
        "Which assistant intent does this utterance express?",
        [],
        "intent",
        eval_split="validation",
        state=lambda r: r["utt"],
        label=idx("intent"),
        keep=lambda r: r["locale"] == "en-US",
        revision="refs/convert/parquet",
        license="ok",
    ),
]
