#!/usr/bin/env bash
# pipeline27 again for the 8B, keeping every evaluated checkpoint so the accuracy-floor rule has something to pick from.
# Same recipe as pipeline21/25 otherwise. usage: pipeline28.sh 1.7b | 8b   (both fit one 80 GB card)
set -u
...
case "${1:?1.7b|8b}" in
  ...
  8b)   N=circuit-8b-v2b;   M=Qwen/Qwen3-8B-Base;   X="--epochs 1"; B=8 ;;
esac
