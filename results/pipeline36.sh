#!/usr/bin/env bash
# circuit v2.1: the v2 mix + rank and match questions + counterfactual and injection groups, at 4,096
...
for f in hard_tier_train hard_tier_eval v2_types_train v2_types_eval v21_types_train v21_types_eval cf_train cf_eval inj_train inj_eval unseen_eval grid_eval hf_eval cmdiy_eval grounded_tools_eval water_calls; do
  [ -f data/$f.jsonl ] || { echo "!!! data/$f.jsonl missing; add it to STAGE"; exit 4; }
done
cat data/publish_train_v2.jsonl data/hard_tier_train.jsonl data/v2_types_train.jsonl data/v21_types_train.jsonl data/cf_train.jsonl data/inj_train.jsonl > data/v21_mix_train.jsonl
