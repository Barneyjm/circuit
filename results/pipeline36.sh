#!/usr/bin/env bash
# circuit v2.1: the v2 mix + rank and match questions, at 4,096 tokens and
# 1 epoch, as pipeline35 (v2), so the new types are the only difference.
# BASE / N pick the model (default 1.7B). Staged, not in git:
#   STAGE="data/hard_tier_train.jsonl data/hard_tier_eval.jsonl data/v2_types_train.jsonl data/v2_types_eval.jsonl data/v21_types_train.jsonl data/v21_types_eval.jsonl data/unseen_eval.jsonl data/grid_eval.jsonl data/hf_eval.jsonl data/cmdiy_eval.jsonl"
set -u
...
for f in hard_tier_train hard_tier_eval v2_types_train v2_types_eval v21_types_train v21_types_eval unseen_eval grid_eval hf_eval cmdiy_eval grounded_tools_eval water_calls; do
  [ -f data/$f.jsonl ] || { echo "!!! data/$f.jsonl missing; add it to STAGE"; exit 4; }
done
cat data/publish_train_v2.jsonl data/hard_tier_train.jsonl data/v2_types_train.jsonl data/v21_types_train.jsonl > data/v21_mix_train.jsonl
...
for pair in "v21_types_eval v21" "v2_types_eval v2" "hard_tier_eval hard" "unseen_eval unseen" "grounded_tools_eval gt" "grid_eval grid" "hf_eval hf" "water_calls water" "cmdiy_eval cmdiy"; do
  set -- $pair
